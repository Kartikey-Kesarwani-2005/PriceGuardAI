...
const QUOTA_ERROR_RE = /(40[123]|429|unauthori[sz]ed|forbidden|invalid[_ ]?(api[_ ]?)?key|api[_ ]?key.*(invalid|expired)|out of (credits?|balance)|insufficient|quota|payment required|billing)/i;

function currentKey() {
    return KEY_POOL[activeKeyIdx] || null;
}
...
function maskKey(key) {
    if (!key) return '(env/config default)';
    return key.length <= 8 ? '****' : `${key.slice(0, 4)}****${key.slice(-4)}`;
}

function rotateKey() {
    if (activeKeyIdx < KEY_POOL.length - 1) {
        activeKeyIdx++;
        console.warn(`[brightdata] Rotating to API key #${activeKeyIdx + 1}/${KEY_POOL.length} (${maskKey(currentKey())})`);
        return true;
    }
    return false;
}
...
async function runBdata(cmd, timeoutMs) {
    if (!KEY_POOL.length) return execCmd(cmd, timeoutMs);
    let lastErr;
    for (let attempt = activeKeyIdx; attempt < KEY_POOL.length; attempt++) {
        const key = KEY_POOL[attempt];
        try {
            const out = await new Promise((resolve, reject) => {
                exec(cmd, { timeout: timeoutMs, windowsHide: true, encoding: 'utf8', maxBuffer: 10 * 1024 * 1024, env: { ...process.env, BRIGHTDATA_API_KEY: key } }, (error, stdout, stderr) => {
            ...
            if (attempt !== activeKeyIdx) {
                console.log(`[brightdata] Now using API key #${attempt + 1} (${maskKey(key)})`);
                activeKeyIdx = attempt;
            }
            return out;
        } catch (e) {
            lastErr = e;
            if (!QUOTA_ERROR_RE.test(e.message || '')) throw e;
            console.error(`[brightdata] Key #${attempt + 1} (${maskKey(key)}) failed: ${String(e.message).slice(0, 120)}`);
        }
    }
    throw new Error(`All ${KEY_POOL.length} Bright Data keys exhausted/failed. Last error: ${lastErr ? lastErr.message : 'unknown'}`);
}
...
function execCmd(cmd, timeoutMs = EXEC_TIMEOUT_MS) {
    return new Promise((resolve, reject) => {
        ...
        if (KEY_POOL.length && !env.BRIGHTDATA_API_KEY) env.BRIGHTDATA_API_KEY = currentKey();
